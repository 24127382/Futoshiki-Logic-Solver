...
class A_StarSolver:
    ...
    def solve(self, input_data: InputData, stop_event: threading.Event = None) -> OutputData:
        ...
        # 2. Run A* solver, passing self so it can update nodes_visited
        result_state = a_star_solver(
            initial_state,
            board,
...
def a_star_solver(
    ...
    solver_instance: A_StarSolver = None
) -> Optional[State]:
    ...
    Args:
        ...
        stop_event: Event to trigger early stopping.
        solver_instance: Reference to the solver class to track nodes visited.
    ...
    while queue:
        # Check for timeout/cancel from the GUI
        if stop_event and stop_event.is_set():
            return None

        # Increment the visited nodes counter
        if solver_instance:
            solver_instance.nodes_visited += 1
